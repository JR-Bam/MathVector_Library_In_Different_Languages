...
func main() {
	...
	vec3, _ := Vecops.Add(vec1, vec2) // Copy Semantics
	vec4, _ := Vecops.Sub(vec1, vec2) // Copy Semantics

	fmt.Println("Vec1:", vec1)
	fmt.Println("Vec2:", vec2)
	dotProduct, _ := Vecops.Dot(&vec1, &vec2)
	fmt.Println("Dot Product:", dotProduct)
	axpyResult, _ := Vecops.Axpy(&vec1, &vec2, -5)
	fmt.Println("AXPY with alpha = -5:", axpyResult)
	fmt.Println("Vec1 + Vec2 =", vec3)
	fmt.Println("Vec1 - Vec2 =", vec4)
	fmt.Println("Vec1 == Vec2 ->", Vecops.SameSize(&vec1, &vec2))
